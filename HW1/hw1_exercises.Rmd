...
dat <- read_csv("contraceptive_use.csv") %>%
  ...
  mutate(Year = as.double((start_date+end_date) / 2), 
         cp = contraceptive_use_modern * 100) %>%
  filter(is_in_union == "Y") %>%
  select(iso,Year,cp)

plot_cp <- function(dat, est, iso_code, CI = 95) { #define CI default value is 95
  est01 <- est %>%
    filter(iso == iso_code)
  dat01 <- dat %>%
    filter(iso == iso_code)
  ...
  plot <- ggplot() +
    ...
    if (!is.na(CI) && CI == 80) { # define CI is not NA and CI is 80
    ...
    else if (is.na(CI)) {  # define CI is NA
    plot <- plot 
  }
  print(plot)
}
...
https://github.com/LinCai0108/repo.git
